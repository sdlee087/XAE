...
class SSWAE_HSIC_dev2(CWAE_GAN_abstract):
    ...
    def pretrain_encoder(self):
        optimizer_main = optim.Adam(sum([list(net.parameters()) for net in self.pretrainable], []), lr = self.lr, betas = (self.beta1, 0.999))
        ...
        for epoch in range(self.encoder_pretrain_step):
            ...
            for i, (data, condition) in enumerate(pretrain_generator):
                n = len(data)
                for net in self.pretrainable:
                    net.zero_grad()

                x = data.to(self.device)
                fake_latent = self.encode(x)[:, 0:self.y_dim]
                y = condition.to(self.device)
                prior_y = self.encode_f1(y)
                loss = (self.k(fake_latent,fake_latent, False) + self.k(prior_y,prior_y, False))/(n*(n-1)) - 2*self.k(fake_latent,prior_y, True)/(n*n)
                obj = loss
                ...
                pp = f"[{i+1:0{ftrep}}/{trep}]  loss: {train_loss_main.avg:.4f}"
                print(pp, end = '\r')
...
class SSWAE_HSIC_gan(CWAE_GAN_abstract):
    def __init__(self, cfg, log, device = 'cpu', verbose = 1):
        super(SSWAE_HSIC_gan, self).__init__(cfg, log, device, verbose)
        self.loss = nn.MSELoss()
        self.disc_loss = nn.BCEWithLogitsLoss()
        ...
        for epoch in range(start_epoch, self.num_epoch):
            ...
            for i, (data, condition) in enumerate(self.train_generator):
                ...
                if self.lamb > 0:
                    ...
                    adv = self.adv_loss(torch.cat((prior_y, prior_z), dim = 1), fake_latent)
                    obj_adv = self.lamb * adv
                    obj_adv.backward()
                    optimizer_adv.step()
                ...
                gan, hsic = self.penalty_loss(fake_latent)
                obj = loss + self.lamb*gan + self.lamb_hsic*hsic
