...
class VAE_MNIST(VAE_abstract):
  def __init__(self, network_info, log, device = 'cpu', verbose = 1):
    super(VAE_MNIST, self).__init__(network_info, log, device, verbose)
    self.d = 32
    d = self.d
    ...
    self.dec = nn.Sequential(
        ...
        nn.Linear(512, 3*3*8*d),
        nn.Unflatten(1, (8*d, 3, 3)),
            
        nn.ConvTranspose2d(8*d, 4*d, kernel_size = 4, stride = 2, padding = 1, output_padding = 1, bias = False),
        nn.ReLU(True),
