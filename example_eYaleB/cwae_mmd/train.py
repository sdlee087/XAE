...
class CWAE_MMD_eYaleB(CWAE_MMD_abstract):
    def __init__(self, cfg, log, device = 'cpu', verbose = 1):
        super(CWAE_MMD_eYaleB, self).__init__(cfg, log, device, verbose)
        self.d = 64
        d = self.d
        ...
        # ).to(device)
        # self.embed_condition2 = nn.Sequential(
        #     nn.Identity(),
        # ).to(device)
        self.enc = nn.Sequential(
            nn.Linear(16*16*d+self.y_dim, d),
            nn.BatchNorm1d(d),
            nn.ReLU(True),
        ...
        #     nn.Linear(self.z_dim + self.y_dim, 4*d),
        #     nn.ReLU(True),
